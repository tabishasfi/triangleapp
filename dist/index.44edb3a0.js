...
})({"lL9hL":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "c81bddf7063b0b5f";
module.bundle.HMR_BUNDLE_ID = "99b7189444edb3a0"; // @flow
...
const handleMouseDown = function(e) {
    ...
    if (shapesArr.length !== 0) for(let j = 0; j < shapesArr.length; j++){
        redrawPrevTriangles(shapesArr[j]);
        if (ctx.isPointInPath(mouseX, mouseY)) {
            shapeIndex = j;
            inShapeDrag = true;
        } else //  In case the click is outside of the Triangle
        isOnlyDrag = true;
    }
    else //  To draw the first Triangle
    isOnlyDrag = true;
};
const handleMouseMove = function(e) {
    ...
    if (inShapeDrag === true) {
        ...
        /*To draw a new Triangle*/ if (isOnlyDrag === false) return;
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        drawTriangle();
    }
};
...
function drawTriangle() {
    if (shapesArr !== null) for(let i = 0; i < shapesArr.length; i++)redrawPrevTriangles(shapesArr[i]);
    ctx.beginPath();
...
function moveTriangle() {
    for(let i = 0; i < shapesArr.length; i++){
        ctx.beginPath();
        ctx.moveTo(shapesArr[i].x0 + shapesArr[i].X, shapesArr[i].y0 + shapesArr[i].Y);
        ctx.lineTo(shapesArr[i].x1 + shapesArr[i].X, shapesArr[i].y1 + shapesArr[i].Y);
        ctx.lineTo(shapesArr[i].x2 + shapesArr[i].X, shapesArr[i].y2 + shapesArr[i].Y);
        ctx.closePath();
...
function handleDoubleClick(e) {
    ...
    dbx = pos.x;
    dby = pos.y;
    if (shapesArr.length !== 0) for(let i = 0; i < shapesArr.length; i++){
...
//# sourceMappingURL=index.44edb3a0.js.map
